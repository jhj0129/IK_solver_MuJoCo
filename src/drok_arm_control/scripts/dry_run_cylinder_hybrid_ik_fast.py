...
def main() -> int:
    ...
    # FAST v0.3:
    # BOX 4처럼 먼 목표에서는 현재 HOME seed에서 full-pose IK를
    # 바로 풀면 local solver가 수렴하지 않을 수 있다.
    # 수렴에 필요한 position-only seed 1회만 복구한다.
    print()
    print(
        "[FAST INITIAL 1/2] Position-only seed "
        "at PICK_PREGRASP"
    )
    ...
    if not initial_position_result["success"]:
        print("[FAIL] FAST position-only initial IK")
    ...
    print()
    print(
        "[FAST INITIAL 2/2] Full-pose orientation "
        "alignment"
    )
    ...
    if not initial_full_result["success"]:
        print("[FAIL] FAST full-pose initial IK")
