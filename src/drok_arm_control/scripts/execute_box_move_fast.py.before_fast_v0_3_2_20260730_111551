...
def create_scaled_block_trajectory(
    ...
    # FAST MODE:
    # 원본 timed path는 약 100 Hz이다. 2.5배 압축 시 모든 점을
    # 그대로 보내면 4 ms 간격이 되어 100 Hz controller보다 조밀하다.
    # 따라서 speed-scale에 맞춰 downsample하고 마지막 점은 유지한다.
    stride = max(1, int(math.ceil(speed_scale)))
    selected_points = list(points[::stride])

    if selected_points[-1] is not points[-1]:
        selected_points.append(points[-1])

    for source in selected_points:
        point = JointTrajectoryPoint()
...
class BoxMoveExecutor(Node):
    ...
    def send_arm_trajectory(
        ...
        # FAST MODE:
        # FollowJointTrajectory 결과 성공을 실행 완료 기준으로 사용한다.
        # 매 segment의 추가 joint error 검증은 생략한다.
        print(f"[FAST 완료] {label}")
    ...
    def send_gripper(
        ...
        # FAST MODE:
        # Action 성공 후 별도의 mm 오차 검증은 생략한다.
        print(f"[FAST 완료] {label}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.5,
        help="MuJoCo FAST 배속. 권장 2.5, 최대 3.0",
    )
...
def main() -> int:
    ...
    try:
        ...
        if home_error > HOME_TOLERANCE_RAD:
            print(
                "[FAST 경고] HOME 밖에서 시작하지만 "
                "현재 자세를 시작점으로 계속 실행합니다."
            )

        current_stage = "GRIPPER_OPEN"
        node.publish_execution_stage(current_stage)

        current_gripper = node.current(GRIPPER_JOINTS)
        open_error = max_difference(
            current_gripper,
            open_target,
        )

        if open_error > 0.003:
            print("[FAST 동작] 그리퍼 빠른 초기 개방")
            node.send_gripper(
                open_target,
                duration=0.35,
                label=current_stage,
        ...
        # FAST MODE: 기존 최소 4.6초 ENTRY를 0.75초로 단축.
        entry_duration = 0.75
        ...
        for block_index, block in enumerate(
            ...
            if start_error > BLOCK_START_TOLERANCE_RAD:
                print(
                    f"[FAST 경고] {segment} 시작 오차 "
        ...
        if retreat_error > ARM_GOAL_TOLERANCE_RAD:
            print(
                "[FAST 경고] PLACE_RETREAT 종료 오차: "
                f"{math.degrees(retreat_error):.3f}°"
            )

        current_stage = "RETURN_HOME_FAST"
        node.publish_execution_stage("RETURN_HOME")
        print("[FAST 동작] 현재 자세에서 HOME으로 직접 복귀")

        current_arm = node.current(ARM_JOINTS)
        home_duration = 1.20

        home_trajectory = create_quintic_trajectory(
            current_arm,
            HOME_Q,
            ARM_JOINTS,
            home_duration,
        ...
        print()
        print("=" * 72)
        print("FAST BOX MOVE RESULT: PASS")
        print("=" * 72)
        print(
