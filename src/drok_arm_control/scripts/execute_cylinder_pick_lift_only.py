...
CONFIRMATION_TEXT = "EXECUTE_PICK_LIFT_ONLY"
...
class PickLiftExecutor(Node):
    def __init__(
        ...
        super().__init__(
            "cylinder_pick_lift_only_executor"
        )
    ...
    def execute(
        ...
        if (
            ...
            raise RuntimeError(
                "PICK_LIFT goal rejected"
            )

        self.get_logger().info(
            "PICK_LIFT_ONLY: goal accepted"
        )
        ...
        if not result_future.done():
            ...
            raise RuntimeError(
                "PICK_LIFT action result timeout"
            )
        ...
        if (
            ...
            raise RuntimeError(
                "PICK_LIFT controller 오류: "
                f"code={result.error_code}, "
                f"message={result.error_string}"
            )

        self.get_logger().info(
            "PICK_LIFT_ONLY: completed"
        )


def create_lift_trajectory(
    block: dict[str, Any],
    speed_scale: float,
) -> JointTrajectory:
...
def main() -> int:
    ...
    trajectory = create_lift_trajectory(
        block,
        speed_scale,
    )
